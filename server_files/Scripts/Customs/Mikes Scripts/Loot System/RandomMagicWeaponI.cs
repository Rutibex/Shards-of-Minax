...
public class RandomMagicWeaponI : BaseWeapon
{
    ...
    [Constructable]
    public RandomMagicWeaponI() : base(GetRandomItemID())
    {
    ...
    private void InitializeWeaponAttributes(BaseWeapon tempWeapon)
    {
        ...
        if (tierChance < 0.05) // Very rare
        {
            ...
            this.MaxDamage = rand.Next(80, 350);
            this.WeaponAttributes.HitLeechMana = rand.Next(10, 21); // Chance to leech mana on hit
            this.WeaponAttributes.HitLeechStam = rand.Next(10, 21); // Chance to leech stamina on hit
            this.WeaponAttributes.HitLeechHits = rand.Next(10, 21); // Chance to leech health on hit
        }
        else if (tierChance < 0.2) // Rare
        {
            this.MinDamage = rand.Next(1, 70);
            this.MaxDamage = rand.Next(70, 200);
            this.WeaponAttributes.HitLightning = rand.Next(20, 41); // Chance to cast Lightning Strike
            this.WeaponAttributes.HitHarm = rand.Next(10, 21); // Chance to cast Harm
        }
        else if (tierChance < 0.5) // Uncommon
        {
            this.MinDamage = rand.Next(1, 50);
            this.MaxDamage = rand.Next(50, 150);
            this.WeaponAttributes.HitFatigue = rand.Next(10, 26); // Chance to fatigue the opponent
            this.WeaponAttributes.HitLeechHits = rand.Next(10, 21); // Chance to leech health on hit
        }
        else // Common
        {
    ...
    public RandomMagicWeaponI(Serial serial) : base(serial) { }
