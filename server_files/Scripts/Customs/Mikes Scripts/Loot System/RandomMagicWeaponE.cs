...
public class RandomMagicWeaponE : BaseWeapon
{
    ...
    // Array of weapon types, feel free to adjust based on available types in your game
    ...
    [Constructable]
    public RandomMagicWeaponE() : base(GetRandomItemID())
    {
        ...
        this.Hue = rand.Next(1, 3001); // Generates a random hue for the weapon
        ...
        tempWeapon.Delete(); // Clean up the temporary weapon instance
    }

    private void InitializeWeaponAttributes(BaseWeapon tempWeapon)
    {
        // Common attributes for all weapons
        this.Attributes.WeaponDamage = rand.Next(25, 50);
        this.Speed = (float)Math.Round(rand.NextDouble() * 9.9 + 0.1, 1);
        this.Attributes.CastSpeed = rand.Next(-1, 2); // Negative for faster casting

        // Tiered attributes based on rarity
        double tierChance = rand.NextDouble();
        if (tierChance < 0.05) // Very rare
        {
            this.MinDamage = rand.Next(15, 60);
            this.MaxDamage = rand.Next(60, 100);
            this.WeaponAttributes.HitFireArea = rand.Next(30, 51);
            this.WeaponAttributes.HitPoisonArea = rand.Next(30, 51);
            this.WeaponAttributes.HitEnergyArea = rand.Next(30, 51);
        }
        else if (tierChance < 0.2) // Rare
        {
            this.MinDamage = rand.Next(10, 45);
            this.MaxDamage = rand.Next(45, 90);
            this.WeaponAttributes.HitPhysicalArea = rand.Next(20, 41);
            this.WeaponAttributes.HitLeechHits = rand.Next(20, 41);
        }
        else if (tierChance < 0.5) // Uncommon
        {
            this.MinDamage = rand.Next(5, 30);
            this.MaxDamage = rand.Next(30, 70);
            this.WeaponAttributes.HitLowerAttack = rand.Next(10, 26);
            this.WeaponAttributes.HitLowerDefend = rand.Next(10, 26);
        }
        else // Common
        {
            this.MinDamage = rand.Next(1, 25);
            this.MaxDamage = rand.Next(25, 50);
            // Common weapons have no special WeaponAttributes
        }

        // Adjust attributes or add new ones based on your game's balance and mechanics
    }
    ...
    private static int GetRandomItemID()
    {
        ...
        tempWeapon.Delete(); // Ensure temporary instance is cleaned up
        return itemID;
    }

    public RandomMagicWeaponE(Serial serial) : base(serial) { }
