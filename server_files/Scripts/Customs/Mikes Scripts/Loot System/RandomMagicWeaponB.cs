...
public class RandomMagicWeaponB : BaseWeapon
{
    ...
    [Constructable]
    public RandomMagicWeaponB() : base(GetRandomItemID())
    {
	...
	private void InitializeWeaponAttributes(BaseWeapon tempWeapon)
	{
		...
		if (tierChance < 0.05) // Very rare
		{
			...
			this.WeaponAttributes.HitLeechMana = rand.Next(10, 21); // High chance of Mana Leech
			this.WeaponAttributes.HitLeechHits = rand.Next(20, 31); // Chance of Hit Point Leech
		}
		else if (tierChance < 0.2) // Rare
		{
			this.MinDamage = rand.Next(1, 70);
			this.MaxDamage = rand.Next(70, 200);
			this.WeaponAttributes.HitDispel = rand.Next(10, 21); // Chance of Dispel
			this.WeaponAttributes.HitLowerAttack = rand.Next(20, 31); // Chance to Lower Attack
		}
		else if (tierChance < 0.5) // Uncommon
		{
			this.MinDamage = rand.Next(1, 50);
			this.MaxDamage = rand.Next(50, 150);
			this.WeaponAttributes.HitFireball = rand.Next(10, 26); // Chance of Fireball
		}
		else // Common
		{
    ...
    public RandomMagicWeaponB(Serial serial) : base(serial) { }
