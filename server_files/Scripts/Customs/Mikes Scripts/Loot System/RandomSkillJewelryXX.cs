...
public class RandomSkillJewelryXX : BaseJewel
{
    ...
    [Constructable]
    public RandomSkillJewelryXX() : base(GetRandomItemID(), Layer.Ring)
    {
    ...
    private void InitializeJewelryAttributes(BaseJewel tempJewelry)
    {
        ...
        if (tierChance < 0.05) // Very rare
        {
			...
			SetRandomSkills(3); // For top-tier jewelry, select 3 random skills
        ...
        else if (tierChance < 0.2) // Rare
        {
			...
			SetRandomSkills(2); // For rare jewelry, select 2 random skills
            // Set other attributes as before
        }
        else if (tierChance < 0.5) // Uncommon
        {
			...
			SetRandomSkills(2); // For uncommon jewelry, still 2 random skills
            // Set other attributes as before
        }
        else // Common
        {
            this.Attributes.Luck = rand.Next(1, 19);
			SetRandomSkills(2); // For common jewelry, 2 random skills
            // Set other attributes as before
        }
    }

    private void SetRandomSkills(int numberOfSkills)
    {
        for (int i = 0; i < numberOfSkills; i++)
        {
            SkillName randomSkill = allSkills[rand.Next(allSkills.Length)];
            int randomValue = Utility.RandomMinMax(0, 100); // Or another range depending on balance
    ...
    public RandomSkillJewelryXX(Serial serial) : base(serial) { }
