...
public class RandomMagicWeaponK : BaseWeapon
{
    ...
    [Constructable]
    public RandomMagicWeaponK() : base(GetRandomItemID())
    {
	...
	private void InitializeWeaponAttributes(BaseWeapon tempWeapon)
	{
		...
		{
			this.MinDamage = rand.Next(30, 50);
			this.MaxDamage = rand.Next(50, 70);
            this.WeaponAttributes.HitFireball = rand.Next(30, 51);
            this.WeaponAttributes.HitMagicArrow = rand.Next(30, 51);
            this.WeaponAttributes.HitEnergyArea = rand.Next(30, 51);
		}
		else if (tierChance < 0.2) // Rare
		{
			this.MinDamage = rand.Next(20, 40);
			this.MaxDamage = rand.Next(40, 60);
			this.WeaponAttributes.HitLeechHits = rand.Next(30, 51); // Chance to leech hits
			this.WeaponAttributes.HitLeechMana = rand.Next(30, 51); // Chance to life mana
		}
		else if (tierChance < 0.5) // Uncommon
		{
			this.MinDamage = rand.Next(10, 30);
			this.MaxDamage = rand.Next(30, 50);
			this.WeaponAttributes.HitDispel = rand.Next(30, 51); // Chance to dispel on hit
            this.WeaponAttributes.HitHarm = rand.Next(30, 51); // Chance to cast harm
		}
		else // Common
		{
...
// but ensure your class structure and other methods properly support these changes.
    ...
    public RandomMagicWeaponK(Serial serial) : base(serial) { }
