...
namespace Server.Mobiles
{
    public class InstantEthereal : EtherealMount
    {
        ...
        public EtherealTypes EthyType
        {
            ...
            set
            {
                if ((int)value > EthyItemTypes.Length)
                    return;
                m_EthyType = value;
                SetMountIDs(EthyItemTypes[(int)value].MountedID, EthyItemTypes[(int)value].RegularID);
            }
        }
        ...
        [Constructable]
        public InstantEthereal(EtherealTypes type) : base(EthyItemTypes[(int)type].RegularID, EthyItemTypes[(int)type].MountedID, 0)
        {
            EthyType = type;
        ...
        private void SetMountIDs(int mountedID, int regularID)
        {
            // Use reflection or another method to set the private/protected read-only properties if necessary
            typeof(EtherealMount).GetProperty("MountedID").SetValue(this, mountedID);
            typeof(EtherealMount).GetProperty("RegularID").SetValue(this, regularID);
        }
    }

    public class DPEthVirtual : EtherealMount
    {
        public DPEthVirtual(int id, int mid) : base(id, mid, 0) { }
