...
namespace Server.Items
{
    ...
    public abstract class BaseClothing : Item, IDyable, IScissorable, IFactionItem, ICraftable, IWearableDurability, IResource, ISetItem, IVvVItem, IOwnerRestricted
    {
		...
		public override void AddNameProperties(ObjectPropertyList list)
		{
			...
			if (ItemPower != ItemPower.None)
			{
				string color = GetPowerColor(ItemPower);
				string powerDescription = GetPowerDescription(ItemPower);

				// Add the colored description without the extra colon
				list.Add(String.Format("<BASEFONT COLOR={0}>{1}</BASEFONT>", color, powerDescription));
			}
		}

		private string GetPowerDescription(ItemPower power)
		{
			switch (power)
			{
				case ItemPower.Minor:
					return "Minor Magic Item";
				case ItemPower.Lesser:
					return "Lesser Magic Item";
				case ItemPower.Greater:
					return "Greater Magic Item";
				case ItemPower.Major:
					return "Major Magic Item";
				case ItemPower.LesserArtifact:
					return "Lesser Artifact";
				case ItemPower.GreaterArtifact:
					return "Greater Artifact";
				case ItemPower.MajorArtifact:
					return "Major Artifact";
				case ItemPower.LegendaryArtifact:
					return "Legendary Artifact";
				case ItemPower.ReforgedMinor:
					return "Reforged Minor Magic Item";
				case ItemPower.ReforgedLesser:
					return "Reforged Lesser Magic Item";
				case ItemPower.ReforgedGreater:
					return "Reforged Greater Magic Item";
				case ItemPower.ReforgedMajor:
					return "Reforged Major Magic Item";
				case ItemPower.ReforgedLegendary:
					return "Reforged Legendary Artifact";
				default:
					return "Unknown Power Level";
			}
		}

		public static string GetPowerColor(ItemPower power)
		{
			switch (power)
			{
				case ItemPower.Minor:
