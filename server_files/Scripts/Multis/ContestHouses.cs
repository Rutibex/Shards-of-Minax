...
namespace Server.Multis
{
    ...
    public class BaseContestHouse : BaseHouse
    {
        ...
        public virtual void AutoAddFixtures()
        {
            ...
            foreach (var entry in components.List.Where(e => e.m_Flags == 0))
            {
                ...
                else
                {
                    ...
                    {
                        BaseDoor door = CreateDoor(entry.m_ItemID, entry.m_OffsetX, entry.m_OffsetY, entry.m_OffsetZ);
                        AddDoor(door, entry.m_OffsetX, entry.m_OffsetY, entry.m_OffsetZ);
                    }
                    else
                    {
        ...
        private BaseDoor CreateDoor(int itemID, int xOffset, int yOffset, int zOffset)
        {
            DoorFacing facing = DetermineDoorFacing(itemID); // Determine the correct facing based on itemID
            BaseDoor door = new DarkWoodDoor(facing); // Replace with the actual door type
            door.MoveToWorld(new Point3D(X + xOffset, Y + yOffset, Z + zOffset), Map);
            return door;
        }

        private DoorFacing DetermineDoorFacing(int itemID)
        {
            // Logic to determine the door facing based on itemID.
            // This is an example and should be replaced with actual logic based on your requirements.
            switch (itemID)
            {
                case 0x0006:
                case 0x0008:
                case 0x000B:
                case 0x001A:
                    return DoorFacing.NorthCW; // Adjust based on actual facing
                case 0x001B:
                case 0x001F:
                case 0x0038:
                    return DoorFacing.SouthCW; // Adjust based on actual facing
                // Add more cases as needed
                default:
                    return DoorFacing.WestCW; // Default facing
            }
        }
    ...
    {
        public override int SignPostID
        {
            get { return 353; }
        }
